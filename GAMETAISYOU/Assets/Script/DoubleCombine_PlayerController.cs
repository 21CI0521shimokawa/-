...
public class DoubleCombine_PlayerController : MonoBehaviour
{
    //はじく===================================
    //float stickX;
    //float stickY;
            ...
            if ((stickLHorizontal < 0) && (stickRHorizontal > 0))
            {
                ...
                if (power >= 1)
                {
                    ...
                    //右
                    {
                        ...
                        GameObject cloneObject1 = Instantiate(mini);
                        cloneObject1.transform.position = position;
                        cloneObject1.GetComponent<Division_PlayerController>().Initialize(Division_PlayerController.LRMode.Right);  //右スティック操作の呼び出し
                    }

                    //左
                    {
                        Vector3 position = new Vector2((float)(transform.position.x - 0.8), transform.position.y);
                        GameObject cloneObject2 = Instantiate(mini);
                        cloneObject2.transform.position = position;
                        cloneObject2.GetComponent<Division_PlayerController>().Initialize(Division_PlayerController.LRMode.Left);   //左スティック操作の呼び出し
                    }
